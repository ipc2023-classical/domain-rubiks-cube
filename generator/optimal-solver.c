...
if (p_metric_data->metric == QUARTER_TURN_METRIC)
   {
   fprintf(stderr, "using quarter turn metric\n");
   ...
   {
   fprintf(stderr, "using face turn metric\n");
...
if (p_user_options->search_limit <= 0)
   fprintf(stderr, "no search limit\n");
else
   fprintf(stderr, "search limit: %d%c\n", p_user_options->search_limit,
                                                p_current_metric->metric_char);

p_user_options->use_symmetry = USE_SYMMETRY;
if (p_user_options->use_symmetry)
   fprintf(stderr, "using symmetry reductions\n");
else
   fprintf(stderr, "not using symmetry reductions\n");

p_user_options->one_solution_only = ONE_SOLUTION_ONLY;
if (p_user_options->one_solution_only)
   fprintf(stderr, "only finding one solution\n");
else
   fprintf(stderr, "finding all solutions\n");
...
while (sdata_struc.found)
      ...
      if (total_found_quot < BACKWARDS_SWITCH_POINT)  /*  search forward  */
         ...
         if (msg_given == 0)
            {
            fprintf(stderr, "     switching to backwards searching\n");
            msg_given = 1;
            }
...
fprintf(stderr, "initializing transformation tables\n");
...
fprintf(stderr, "initializing distance table ... this will take several minutes\n");
...
for (ii = 0; ii < 12; ii++)
    ...
    if (jj == 24)
       {
       if (give_err_msg)
          fprintf(stderr, "invalid edge cubie: %s\n", edge_str[ii]);
       stat = 1;
       }
    }
...
for (ii = 0; ii < 8; ii++)
    ...
    if (jj == 24)
       {
       if (give_err_msg)
          fprintf(stderr, "invalid corner cubie: %s\n", corner_str[ii]);
       stat = 1;
       }
    }
...
if (perm_n_check(24, p_cube->edges))
   {
   if (give_err_msg)
      fprintf(stderr, "bad edge cubies\n");
   stat = 1;
   }
...
if (perm_n_check(24, p_cube->corners))
   {
   if (give_err_msg)
      fprintf(stderr, "bad corner cubies\n");
   stat = 1;
   }
...
if (p_current_options->use_symmetry)
   ...
   if (count > 1)
      fprintf(stderr, "position has %d-fold symmetry (subgroup #%d)\n", count,
                                                           p_cube->sym_subgrp);
   else
      fprintf(stderr, "asymmetric position\n");
   }
else
...
fprintf(stderr, "\nSolution:");
for (ii = 0; ii < count; ii++)
    fprintf(stderr, " %s", twist_string[turn_list[ii]]);
fprintf(stderr, "\n");
if (p_current_metric->metric == QUARTER_TURN_METRIC)
   fprintf(stderr, "  (%dq*, %df)\n", q_length, f_length);
else
   fprintf(stderr, "  (%df*, %dq)\n", f_length, q_length);
fflush(stderr);

printf(";; Cost: %d\n", count);
for (ii = 0; ii < count; ii++)
    printf("(%s)\n", twist_string[turn_list[ii]]);
fflush(stdout);
...
for (ii = 3; ii >= 0; ii--)
    ...
    if (started)
       {
       if (digits[ii] >= 100)
          fprintf(stderr, "%3d", digits[ii]);
       else if (digits[ii] >= 10)
               fprintf(stderr, "0%2d", digits[ii]);
       else
          fprintf(stderr, "00%1d", digits[ii]);
       }
    else
       {
       if (digits[ii] >= 100)
          ...
          started = 1;
               ...
               {
               fprintf(stderr, " %2d", digits[ii]);
               started = 1;
               }
       else if ((digits[ii] >= 1) || (ii == 0))
               {
               fprintf(stderr, "  %1d", digits[ii]);
               started = 1;
               }
       else
          fprintf(stderr, "   ");
...
if (cube_is_solved(p_cube))
   {
   fprintf(stderr, "cube is already solved!\n");
   return;
   }
...
for (ii = start_depth; ii <= search_limit; ii += p_current_metric->increment)
    ...
    if ((p_current_options->one_solution_only == 0) || (sol_found == 0))
       {
       fprintf(stderr, "depth %2d%c completed  (", ii, p_current_metric->metric_char);
